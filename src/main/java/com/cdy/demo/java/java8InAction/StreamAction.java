package com.cdy.demo.java.java8InAction;
...
public class StreamAction {
    ...
    public static void main(String[] args) {
        ...
        java.util.stream.Stream<Dish> menuStream = menu.stream();
...
class StreamForker<T> {
    ...
    private final Map<Object, Function<java.util.stream.Stream<T>, ?>> forks =
            new HashMap<>();

    public StreamForker(java.util.stream.Stream<T> stream) {
        this.stream = stream;
    }

    public StreamForker<T> fork(Object key, Function<java.util.stream.Stream<T>, ?> f) {
        forks.put(key, f);
        return this;
    }
    ...
    private Future<?> getOperationResult(List<BlockingQueue<T>> queues,
                                         Function<java.util.stream.Stream<T>, ?> f) {
        BlockingQueue<T> queue = new LinkedBlockingQueue<>();
        queues.add(queue);
        Spliterator<T> spliterator = new BlockingQueueSpliterator<>(queue);
        java.util.stream.Stream<T> source = StreamSupport.stream(spliterator, false);
